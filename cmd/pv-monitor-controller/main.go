...
func main() {
	...
	supportGetVolumeHealth, err := supportGetVolumeHealth(ctx, csiConn)
	if err != nil {
		klog.Error(err.Error())
		os.Exit(1)
	}
...
func supportGetVolume(ctx context.Context, csiConn *grpc.ClientConn) (supportGetVolume bool, err error) {
	...
	for _, cap := range rsp.GetCapabilities() {
		...
		alphaFeature := cap.GetAlphaFeature()
		if alphaFeature == nil {
			continue
		}
		t := alphaFeature.GetType()
		if t == csi.ControllerServiceCapability_AlphaFeature_GET_VOLUME {
			return true, nil
		}
	}
...
// TODO: move this to csi-lib-utils
func supportGetVolumeHealth(ctx context.Context, csiConn *grpc.ClientConn) (supportGetVolumeHealth bool, err error) {
	client := csi.NewControllerClient(csiConn)
	req := csi.ControllerGetCapabilitiesRequest{}
	rsp, err := client.ControllerGetCapabilities(ctx, &req)
	if err != nil {
	...
	for _, cap := range rsp.GetCapabilities() {
		...
		t := alphaFeature.GetType()
		if t == csi.ControllerServiceCapability_AlphaFeature_VOLUME_HEALTH {
			return true, nil
		}
	}
