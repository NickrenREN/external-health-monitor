...
func (checker *PVHealthConditionChecker) CheckControllerVolumeStatus(pv *v1.PersistentVolume) error {
	...
	condition, errMessages, err := checker.csiPVHandler.ControllerVolumeChecking(ctx, volumeHandle)
	if err != nil {
		return err
	}

	if condition == util.VolumeHealthy {
		klog.V(5).Infof("volume is healthy, do not send any pvc events")
		return nil
	}

	// At the first stage, we just send PVC events
	for _, errMessage := range errMessages {
		checker.eventRecorder.Event(pvc, v1.EventTypeWarning, "PV is not healthy(detected by monitor controller)", errMessage)
	}
...
func (checker *PVHealthConditionChecker) CheckNodeVolumeStatus(kubeletRootPath string, supportStageUnstage bool, pv *v1.PersistentVolume, pod *v1.Pod) error {
	...
	condition, errMessages, err := checker.csiPVHandler.NodeVolumeChecking(ctx, volumeHandle, volumePath, stagingTargetPath)
	if err != nil {
		return err
	}
	...
	// At the first stage, we just send PVC events
	for _, errMessage := range errMessages {
		checker.eventRecorder.Event(pvc, v1.EventTypeWarning, "PV is not healthy(detected by monitor agent)", errMessage)
	}
